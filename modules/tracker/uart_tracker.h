 ...
 *       采用双阈值滞回死区: 进入阈值决定何时启动, 退出阈值决定何时停止,
 *       形成滞回环防止边界抖动.
 *
...
typedef struct {
    int32_t enter_zone_x;      /* X轴进入阈值 (像素), |error|>=此值开始动作 */
    int32_t exit_zone_x;       /* X轴退出阈值 (像素), |error|<=此值停止 (需<enter才能形成滞回) */
    int32_t enter_zone_y;      /* Y轴进入阈值 (像素) */
    int32_t exit_zone_y;       /* Y轴退出阈值 (像素) */
