 ...
 *
 * 功能: 接收 K230 坐标, 双阈值滞回死区+比例控制双轴步进电机跟踪目标
...
/* 双阈值死区状态: 追踪电机是否已启动 (用于滞回判断) */
static bool g_motor_a_running;  /* true=上次在动作, false=上次已停止 */
static bool g_motor_b_running;
...
/*
 * 计算单个轴的电机方向和频率 (双阈值滞回死区).
 *   error: 像素偏差 (正=目标偏右/下, 负=偏左/上)
 *   enter_zone: 进入阈值 (像素), |error|>=此值开始动作
 *   exit_zone:  退出阈值 (像素), |error|<=此值才停止 (需<enter形成滞回)
 *   kp: 比例系数 (Hz/px)
 *   min_freq / max_freq: 频率边界
 *   running: 指向状态变量, true=上次在动作(用exit), false=上次停止(用enter)
 *   out_dir: 输出方向 (-1=停止, 0=CCW, 1=CW)
 *   out_freq: 输出频率
 */
static void motor_compute(int32_t error,
                          int32_t enter_zone, int32_t exit_zone,
                          ...
                          int32_t *out_dir, int32_t *out_freq)
{
    ...
    /* 双阈值滞回: 已运行时用退出阈值, 未运行时用进入阈值 */
    int32_t threshold = *running ? exit_zone : enter_zone;
    ...
    /* 超出阈值 → 启动/保持动作 */
    *running = true;
    int32_t effective = abs_err - threshold;
...
void tracker_init(const TrackerConfig *cfg)
{
    ...
    g_motor_a_freq    = 0;
    g_motor_b_freq    = 0;
    g_motor_a_running = false;  /* 双阈值滞回状态 */
    g_motor_b_running = false;
    g_new_data        = false;
...
void tracker_update(void)
{
    ...
    /* ---- 3. 双阈值滞回死区 + 比例控制 (仅在新数据到达时更新, 避免旧误差重复驱动) ---- */
    if (g_new_data) {
        g_new_data = false;

        int32_t dir_a, freq_a, dir_b, freq_b;

        motor_compute(g_coord.error_x,
                      g_cfg.enter_zone_x, g_cfg.exit_zone_x,
                      g_cfg.kp_x, g_cfg.min_freq, g_cfg.max_freq,
                      &g_motor_a_running,
                      &dir_a, &freq_a);
        motor_compute(g_coord.error_y,
                      g_cfg.enter_zone_y, g_cfg.exit_zone_y,
